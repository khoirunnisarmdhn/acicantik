    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-100 print:shadow-none print:border-none">
                ...
                {{-- Header --}}
                <div class="bg-emerald-600 px-8 py-8 text-white flex justify-between items-center print:bg-white print:text-black print:border-b print:px-0">
                    <div>
                        <h2 class="text-3xl font-black tracking-tighter">DETAIL PENERIMAAN KAS</h2>
                        <p class="text-sm opacity-80 font-mono mt-1">{{ $data->no_form }}</p>
                    </div>
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-2 gap-8 mb-8">
                        <div>
                            ...
                            <p class="text-lg font-bold text-gray-800">{{ \Carbon\Carbon::parse($data->tanggal)->translatedFormat('d F Y') }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Metode Pembayaran</p>
                            <p class="text-lg font-bold text-emerald-600">{{ $data->nama_metode_bayar }}</p>
                        </div>
                    </div>

                    {{-- Info Utama --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-2xl border border-gray-100 mb-8">
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-bold text-indigo-600">{{ $data->nama_proyek ?? 'Penerimaan Umum / Non-Proyek' }}</p>
                        </div>
                        @if($data->nama_termin)
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tipe Termin</p>
                            <p class="font-bold text-gray-700">{{ $data->nama_termin }} ({{ $data->persentase }}%)</p>
                        </div>
                        @endif
                        @if($data->nilai_kontrak)
                        <div>
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Nilai Kontrak</p>
                            <p class="font-bold text-gray-700">Rp {{ number_format($data->nilai_kontrak, 0, ',', '.') }}</p>
                        </div>
                        @endif
                    </div>
                    ...
                    <div class="flex justify-between items-center border-t-2 border-dashed border-gray-100 pt-8">
                        <div>
                            <p class="text-xs font-bold text-gray-400 uppercase">Total Penerimaan</p>
                            <h3 class="text-4xl font-black text-emerald-600">Rp {{ number_format($data->nominal, 0, ',', '.') }}</h3>
                        </div>
                ...
                <div class="bg-gray-50 px-8 py-4 flex justify-between items-center border-t border-gray-100 print:hidden">
                    <a href="{{ url()->previous() }}" class="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-gray-700 transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
